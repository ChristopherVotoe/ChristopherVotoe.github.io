...
export function SessionPanel({ session, active, onBegin, onRender, onDelete }: {
  ...
  return <section className="session-panel" aria-label="Recording challenge">
    <div className="session-heading"><div><p className="eyebrow">YOUR FIVE-MOVE CHALLENGE</p><h2>{view.phase === "complete" ? "You made a dance." : view.phase === "review" ? "Five moves. Ready for the reveal?" : "Make a little magic."}</h2></div><span>{count} / 5 clips</span></div>
    ...
    {["ready", "paused"].includes(view.phase) && <div className="session-actions"><button className="primary" disabled={!active} onClick={() => onBegin()}>{view.phase === "paused" ? "Resume recording challenge" : "Start recording challenge"}</button><p>Records five short camera clips when you match the gestures. No microphone. Clips stay in this tab until you choose “Create my video,” which uploads them to this app’s server for rendering. Temporary server files are removed after rendering or cancellation.</p>{!active && <p>Enable your camera above to continue.</p>}</div>}
    {playing && <div className="session-actions"><p role="status">{view.phase === "recording" ? "● Recording — hold that pose!" : view.phase === "countdown" ? `Get ready: ${view.countdown}` : `Match ${gestures[dance.steps[view.step]].name} with both hands.`}</p><button className="secondary" onClick={session.pause}>Pause challenge</button></div>}
    {["review", "complete"].includes(view.phase) && <>
      ...
      {view.phase === "review" && <div className="session-actions"><button className="primary" onClick={onRender}>Create my video ↗</button><p>Upload these five clips to create a 10-second MP4 with our original backing track. The rendered video returns to this tab; the server keeps no session or video archive.</p></div>}
    </>}
    {view.phase === "processing" && <div className="render-status" role="status"><h3>Putting your moves to music…</h3><p>Keep this tab open. Rendering can take a minute or two.</p><progress aria-label="Rendering video" /><button className="secondary" onClick={session.cancelRender}>Cancel rendering</button></div>}
    {view.output && <div className="reveal"><video src={view.output} controls playsInline aria-label="Your final dance video" /><div className="session-actions"><a className="primary" href={view.output} download="hand-signal-dance.mp4">Download MP4 ↓</a><p>Those five gestures were a dance all along. Press play to watch or replay with sound.</p></div></div>}
