...
import { EmailConsent, VideoPrivacy, VideoEmailStatus } from "./VideoEmail";
...
export function SessionPanel({ session, active, onBegin, onRender, onDelete }: {
  ...
  return <section className="session-panel" aria-label="Recording challenge">
    ...
    <EmailConsent email={session.email} disabled={playing || view.phase === "processing"} />
    {["ready", "paused"].includes(view.phase) && <div className="session-actions"><button className="primary" disabled={!active || (session.email.enabled && !session.email.consent)} onClick={() => onBegin()}>{view.phase === "paused" ? "Resume recording challenge" : "Start recording challenge"}</button><p>Records two short camera clips as you follow the animations, after a two-second countdown. No microphone. <VideoPrivacy /> Keep this tab visible during video creation.</p>{!active && <p>Enable your camera above to continue.</p>}</div>}
    ...
    {["review", "complete"].includes(view.phase) && <>
      ...
      {view.phase === "review" && <div className="session-actions"><button className="primary" onClick={onRender}>Create my video ↗</button><p>Create an 11-second video on your device, with your movements sped up to 86 BPM and repeated to “Join Us for a Bite” (0:24–0:35). Download as MP4 or WebM, depending on your browser. <VideoPrivacy /></p></div>}
    </>}
    ...
    {view.output && <VideoEmailStatus email={session.email} />}
