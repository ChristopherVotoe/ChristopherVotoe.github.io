...
export function SessionPanel({ session, active, onBegin, onRender, onDelete }: {
  ...
  return <section className="session-panel" aria-label="Recording challenge">
    ...
    {["review", "complete"].includes(view.phase) && <>
      ...
      {view.phase === "review" && <div className="session-actions"><button className="primary" onClick={onRender}>Create my video ↗</button><p>Upload these two clips to create a 11-second MP4, with your movements sped up to 86 BPM and repeated to “Join Us for a Bite” (0:24–0:35). The rendered video returns to this tab; the server keeps no session or video archive.</p></div>}
    </>}
    ...
    {view.output && <div className="reveal"><video src={view.output} controls playsInline aria-label="Your final dance video" /><div className="session-actions"><a className="primary" href={view.output} download="hand-signal-dance.mp4">Download MP4 ↓</a><p>Your two movements, stitched together. Press play to watch or replay. Play with sound to hear “Join Us for a Bite.”</p></div></div>}
