...
import { VideoEmailStatus } from "@/components/VideoEmail";
...
export default function ResultsPage() {
  ...
  return <main className="results-page"><SiteHeader current="results" />
    <section className="flow-intro"><p className="eyebrow">YOUR RESULT</p><h1>{ready ? "Your video is ready." : hasClips ? "One moment…" : "Ready for your test?"}</h1><p>{ready ? "Your two movements, with a little rhythm added." : hasClips ? "Your test is complete. We’re preparing your video automatically." : "Start a test to see your video here. Recordings clear when you reload or close this tab."}</p></section>
    {ready ? <section className="reveal"><video src={session.view.output!} autoPlay controls playsInline aria-label="Your final dance video" onCanPlay={(event) => { void event.currentTarget.play().catch(() => setPlayBlocked(true)); }} onPlay={() => setPlayBlocked(false)} />{playBlocked && <p className="session-note">Your browser requires a tap on Play to start the video with sound.</p>}<div className="session-actions">{session.view.outputFile && <VideoDownload url={session.view.output!} file={session.view.outputFile} />}<Link className="secondary" href="/">Back home</Link></div><VideoEmailStatus email={session.email} /></section> : hasClips ? <section className="session-panel">
