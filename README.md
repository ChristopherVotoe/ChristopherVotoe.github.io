...
To run the supplied Resend Hello World example, see the [local email test instructions](workers/video-email/README.md#optional-send-the-hello-world-example-locally). The command is `npm run email:test`; it reads a private key from the ignored `.env.email.local` file.
